    ...
    class FB
    {
      ...
      // duyệt đồ thị
        ...
        public Bitmap DuongDiPic(int bd, int kt,Image anh)
        {
            ...
            int i = Truoc[kt];
            a = LineTo(dsve[kt], dsve[Truoc[kt]], a, matrantrongso[kt, Truoc[kt]], Color.GreenYellow);
            while (i != bd)
            {
                a = LineTo(dsve[i], new PointF(dsve[Truoc[i]].X, dsve[Truoc[i]].Y), a, matrantrongso[i , Truoc[i]], Color.GreenYellow);
                i = Truoc[i];
            }
            return a;
        }
            ...
            for (int i = 1; i <= sodinh; i++)
            {
                for (int j = 1; j <= sodinh; j++)
                {
                    if (matrantrongso[i - 1, j - 1] != vocung)
                    {
                            ...
                            a = LineTo(dsve[i - 1], dsve[j - 1], a, matrantrongso[i - 1, j - 1], Color.Blue);
